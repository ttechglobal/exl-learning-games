"use client";
// FILE: src/app/(admin)/admin/content/[topicId]/MathTopicClient.tsx
...
function ConceptRow({
  ...
  const handlePublish = async () => {
    if (!concept.guidedQuestions?.length && !concept.practiceQuestions?.length && !concept.challengeQuestions?.length) {
      setPublishResult("✗ No questions imported yet. Import questions first.");
      return;
    }
    setPublishing(true);
    setPublishResult(null);
    try {
      // Use the dedicated maths publish endpoint — passes concept index for targeted publish
      const res = await fetch(`/api/maths-topics/${topic.id}/publish`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ conceptIndex: idx }),
      });
      const json = await res.json();
      if (res.ok) {
        const r = json.published?.[0];
        setPublishResult(`✓ Published — ${r?.missionCount ?? 0} missions live in ${r?.gameSlug}`);
        onSave({ ...concept, status: "approved" });
      } else {
    ...
    <div style={{
      ...
      {open && (
        <div style={{ padding: "14px 16px", background: "var(--surface-0)", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {/* Step 3: Publish directly to game */}
          {(guidedCount + practiceCount + challengeCount) > 0 && (
            <div style={{
              background: "var(--surface-1)", borderRadius: 8, padding: 12,
              border: "1px solid var(--border)",
            }}>
              <div style={{ fontSize: "0.65rem", fontWeight: 700, color: ACCENT, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>Step 3</div>
              <div style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text)", marginBottom: 6 }}>Publish to the game</div>
              <div style={{ fontSize: "0.72rem", color: "var(--text-4)", marginBottom: 10 }}>
                Pushes all imported questions directly to game slug: <code style={{ fontFamily: "monospace", color: ACCENT }}>{topic.game_slug || topic.topic_id}</code>
              </div>
              <button
              ...
              {publishResult && (
                <div style={{
                  ...
                  color: publishResult.startsWith("✓") ? "#059669" : "#f87171",
                  background: publishResult.startsWith("✓") ? "rgba(5,150,105,0.08)" : "rgba(248,113,113,0.08)",
...
export default function MathsTopicClient({ topic: initialTopic }: { topic: Topic }) {
  ...
  const updateConcept = async (idx: number, updated: Concept) => {
    const newConcepts = topic.concepts.map((c: Concept, i: number) => i === idx ? updated : c);
  ...
  const builtCount = topic.concepts.filter((c: Concept) => c.status === "built" || c.status === "approved").length;
    ...
    <div style={{ padding: "24px 28px", maxWidth: 900 }}>
      ...
      {(topic.misconceptions ?? []).length > 0 && (
        <div style={{ marginTop: 32 }}>
          ...
          {topic.misconceptions!.map((m: { belief: string; correction: string; stage: string }, i: number) => (
            <div key={i} style={{
