...
// One character per subject. Add more here as subjects grow.
// coatColor / labelColor drive the SVG fills without forking the component.
...
const FALLBACK_CHARACTER = CHARACTERS.chemistry;

// ─── Briefing lines ──────────────────────────────────────────────────────────
// Each game's lines come from resolveMissionBriefing (already in codebase).
// We split the paragraph on ". " to create natural dialogue beats.
// Minimum 1 line, maximum 4.
...
function splitIntoLines(text: string): string[] {
  ...
  // Group into ≤4 cards of 1–2 sentences each
  const cards: string[] = [];
  let i = 0;
  ...
  // Remaining sentences go on the last card
...
function SceneBackground({ subject }: { subject: string }) {
  ...
  return (
    ...
    >
      ...
      {/* Background figure (distant, very muted) */}
...
function CharacterFigure({ subject }: { subject: string }) {
...
export interface NarrationScreenProps {
  ...
  subject: string;
  mission: MissionRow;
  /** Called when the student taps "Begin mission" on the final card */
  onStart: () => void;
}

// ─── Main component ──────────────────────────────────────────────────────────

export function NarrationScreen({ gameSlug, subject, mission, onStart }: NarrationScreenProps) {
  const character = CHARACTERS[subject] ?? FALLBACK_CHARACTER;

  // Build dialogue lines from the mission briefing paragraph
  ...
  const [baseLines] = useState<string[]>(() => {
    ...
    // Final card always prompts the action
    return [
      ...parsed,
