...
/**
 * StepwiseEngine.tsx
 * Change of Subject — Stepwise Flow
 *
 * Reads missions from config.shared._allMissions (injected by PlayClient).
 * Filters by difficulty: EASY → Guided, MEDIUM → Practice, HARD → Challenge.
 * Renders all math via KaTeX (install: npm install katex).
 * Coach text supports <strong> and <em> HTML tags.
 */
...
import styles from "./StepwiseEngine.module.css";
...
export interface StepChoice {
  icon: string;
  label: string;  // plain text — rendered as-is in choice button
  sub: string;
  correct: boolean;
}

export interface QuestionStep {
  trailLabel: string;   // plain text
  resultEq: string;     // LaTeX
  coach: string;        // HTML (supports <strong>, <em>)
  coachWrong: string;   // HTML
  hint: string;         // plain text
  choiceQuestion: string; // plain text
  choices: StepChoice[];
}

export interface StepwiseQuestion {
  goal: string;         // plain text, e.g. "Make t the subject"
  formula: string;      // LaTeX, e.g. "v = u + at"
...
export default function StepwiseEngine({ config, onComplete }: EngineRuntimeProps) {
  ...
  const questions =
    ...
    state.mode === "challenge" ? challengeQ :
                                 challengeQ; // mastery uses HARD for now
      ...
      <div className={styles.scroll}>
        ...
        <div className={styles.missionCard}>
          <div className={styles.mcTop}>
            <div>
              <div className={styles.mcCase}>{currentQ.topic} · Formula</div>
              <div className={styles.mcFormula}>
                <KaTeX tex={currentQ.formula} />
              </div>
      ...
      {livesOut && (
        <div className={styles.livesOut}>
          ...
          <div className={styles.livesOutSub}>
            The answer was:{" "}
            <span className={styles.livesOutAnswer}><KaTeX tex={currentQ.finalAnswer} /></span>
...
function HubScreen({
  state, dispatch, hasPractice, hasChallenge,
}: {
  state: EngineState;
  ...
  const modes: { id: StepMode; icon: string; name: string; desc: string; color: string; locked: boolean }[] = [
    { id: "guided",    icon: "📖", name: "Guided Learning", desc: "Ms. Chidera coaches every step. Learn why each operation works.",    color: "#f5a623", locked: false },
    { id: "practice",  icon: "⚡", name: "Practice",         desc: "No coach. Work it out yourself. Free hint available on tap.",        color: "#6c28e0", locked: !hasPractice },
    { id: "challenge", icon: "🔥", name: "Challenge",         desc: "No help. Lose a heart for every wrong pick. Exam conditions.",       color: "#e03c28", locked: !hasChallenge },
    { id: "mastery",   icon: "🏅", name: "Mastery",           desc: "Application questions. No scaffolding. WAEC / JAMB level.",         color: "#1a7a4a", locked: true },
  ];
  ...
  return (
    <div className={styles.hubRoot}>
      ...
      <div className={styles.hubContent}>
        <div className={styles.hubKicker}>Change of Subject</div>
        <h1 className={styles.hubTitle}>
          Stepwise <span className={styles.hubTitleAccent}>Flow</span>
        </h1>
        <p className={styles.hubDesc}>
          Rearrange formulae step by step. Each correct move builds the trail.
        </p>
        ...
        <div className={styles.modeList}>
          {modes.map(m => (
            <button
              key={m.id}
              className={`${styles.modeBtn} ${state.mode === m.id ? styles.modeBtnActive : ""} ${m.locked ? styles.modeBtnLocked : ""}`}
  ...
  return (
    <div className={styles.selfSolveRoot}>
      ...
      <div className={styles.selfSolveContent}>
        ...
        <div className={styles.ssFormulaWrap}><KaTeX tex={question.formula} block /></div>
        ...
        <div className={styles.ssBtns}>
          <button className={`${styles.ssBtn} ${styles.ssBtnSecondary}`} onClick={() => dispatch({ type: "SELF_SOLVE_SKIP" })}>
      ...
      <div className={styles.reviewContent}>
        ...
        <div className={styles.reviewTrail}>
          ...
          <div className={styles.reviewTrailRow}>
            <div className={styles.reviewTrailDot} style={{ background: "rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.4)", fontSize: 10 }}>↓</div>
            <div className={styles.reviewTrailBody}>
              <div className={styles.reviewTrailLabel}>Starting formula</div>
          ...
          <div className={styles.reviewTrailRow}>
            ...
            <div className={styles.reviewTrailBody}>
              <div className={styles.reviewTrailLabel}>Final answer</div>
...
function CompleteScreen({
  ...
  const msgs: Record<StepMode, { icon: string; title: string; sub: string }> = {
    guided:    { icon: "🎉", title: "You got it!",        sub: "You followed every step. That's the complete rearrangement." },
